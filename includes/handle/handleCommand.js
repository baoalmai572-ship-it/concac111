module.exports = function ({ api, models, Users, Threads, Currencies }) {
  ...
  // CONFIG LIMIT COMMAND
  const commandGroups = {
      game: ['1key', '1hoso', 'tinhdiem', 'tinhdiemlogo'],
      anti: ['anti']
  };
  // END CONFIG LIMIT COMMAND
  ...
  return async function ({ event }) {
    ...
    // ===== QTVONLY (PHIÊN BẢN HOÀN CHỈNH) =====
    try {
        const qtvonlyPath = path.join(__dirname, '..', '..', 'modules', 'commands', 'cache', 'qtvonly.json');
        if (fs.existsSync(qtvonlyPath)) {
            const qtvonlyData = JSON.parse(fs.readFileSync(qtvonlyPath, 'utf8'));
            if (qtvonlyData.qtvbox && qtvonlyData.qtvbox[threadID] === true) {
                const isBotAdmin = ADMINBOT.includes(senderID) || NDH.includes(senderID);
                if (!isBotAdmin) {
                    let threadInfoForQTV;
                    try {
                        threadInfoForQTV = await Threads.getInfo(threadID);
                    } catch (error) {
                        threadInfoForQTV = threadInfo.get(threadID);
                    }

                    if (threadInfoForQTV && threadInfoForQTV.adminIDs) {
                        const isGroupAdmin = threadInfoForQTV.adminIDs.some(item => item.id == senderID);
                        if (!isGroupAdmin) {
                            return api.sendMessage('❗️ Nhóm đang bật chế độ QTV Only. Chỉ Quản trị viên nhóm và Admin Bot mới có thể sử dụng lệnh.', threadID, messageID);
                        }
                    }
                }
            }
        ...
        console.error(`[QTVONLY-ERROR] Lỗi khi xử lý qtvonly: ${e}`);
    }
    // ===== KẾT THÚC QTVONLY =====
